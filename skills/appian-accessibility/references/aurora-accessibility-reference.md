# Aurora Design System — Accessibility Reference

This reference consolidates the accessibility guidance available from the Aurora MCP server (Appian Aurora Design System). Use it as a lookup when auditing or building accessible Appian SAIL interfaces.

## Source

All content here is derived from the Aurora Design System documentation:
- **Repository**: https://github.com/appian-design/aurora
- **Live docs**: https://appian-design.github.io/aurora/
- **MCP server**: https://github.com/appian-design/aurora-mcp

The Aurora MCP makes this information queryable at development time inside AI coding assistants.

---

## Accessibility Checklist Summary

The Aurora checklist defines success criteria across these categories. Each criterion specifies a SAIL Testing method (inspectable via component parameters), plus screen reader, keyboard, DOM, and visual testing approaches. The rules below focus on what can be verified programmatically from SAIL source or rendered component trees.
...
These cannot be checked programmatically from SAIL source but are part of the full Aurora checklist:
...
## Design Patterns from Aurora

The Aurora MCP also provides design patterns with built-in accessibility considerations:
...
4. **For the full checklist** including manual testing methods, fetch the latest from:
   ```
