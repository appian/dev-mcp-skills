...
- **Concatenating `a!richTextIcon()` with text using `&`** — writing `a!richTextIcon(icon: "plus") & " Add Vehicle"` inside `a!richTextItem(text: ...)` renders as raw `@attributes` at runtime. Rich text icons are components, not strings. Instead, use separate items in the `value` list: `{ a!richTextIcon(icon: "plus"), a!richTextItem(text: " Add Vehicle", link: ...) }`.
