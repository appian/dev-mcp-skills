...
|---|---|---|---|
| **CRITICAL** | Affects multiple objects or production data | Application delete, Record Type delete, Process Model delete | Typed confirmation required |
| **HIGH** | Affects single object with potential dependencies | Field delete, Relationship delete, Expression Rule delete | Yes/No confirmation |
| **MEDIUM** | Affects single object, limited dependencies | View delete, Action delete, Constant delete | Yes/No confirmation |
...
4. **Check dependencies** (object-specific)
   ...
   **Current limitation:** Automated dependency analysis requires manual inspection. Perform basic structural checks:
   - Applications: List contained objects (record types, interfaces, etc.) via `listApplicationObjects`
   - Record Types: Count fields, relationships, views, actions via respective list operations
   - Fields: Check if used in relationships (via `listRecordTypeRelationships`)
   - Expression Rules: Cannot detect what references this rule automatically
   
   For comprehensive impact analysis, state: **"Manual verification recommended via Appian Designer dependency view (View Dependents / View Precedents)"**
   
   **Future enhancement:** If a dependency analysis tool becomes available (e.g., `getObjectDependents`, `getObjectPrecedents`), integrate it here to provide complete impact analysis automatically.

5. **Present to user with consequences**
   - CRITICAL risk: Require typed confirmation
   - HIGH/MEDIUM risk: Require Yes/No confirmation
   
   **Example for CRITICAL risk:**
   ```
   You are about to DELETE the "Case Management" application (UUID: abc-123).
   
   This will permanently delete:
   - 3 record types (Case, Customer, Status)
   - 12 interfaces
   - 8 expression rules
   - 2 process models
   - 1 site
   ...
   To confirm, type: DELETE Case Management
   ```
...
When presenting delete confirmation for record types, clarify what is actually removed:
...
- [N] data records remain in the database but are no longer accessible via Appian
- To drop the table, manual database administration is required

Impact on other objects:
- Fields referencing this record type will become orphaned
- Interfaces/expressions using this record type will break
- Process models writing to this record type will fail
